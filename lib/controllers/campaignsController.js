...
const UserAuth = function(db, reqSessionId) {
  let querryString =
  (`
    SELECT name
    FROM users
  ...
  return db.query(querryString, [req.session.id])
};

// delete campaign by an influencer using their own id
...
// set status to 'removed' by the brand manager
const updateTask =  function(db, userObject) {
  const querryString =
  (`
  UPDATE tasks
  SET status = 'removed'
    FROM campaigns,  campaign_details, brands, brand_managers, users
    WHERE campaign_id = campaigns.id AND campaign_detail_id = campaign_details.id AND campaign_details.brand_id = brands.id AND brand_managers.brand_id = brand_managers.id AND user_id = users.id
  AND tasks.id = $1 AND user_id = $2;
  `)

  return db.query(querryString, [userObject.task.id, userObject.user.id])
};
