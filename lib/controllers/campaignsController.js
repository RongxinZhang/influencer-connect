...
const deleteInfluencerCampaign = function (userObj) {
  const querryString = `
    ...
    `;
  console.log(userObj);
  return db.query(querryString, [userObj.compaignId, userObj.userId]);
};

// delete Campaign by the brand rep
const deleteCampaign = function (compaignId) {
  const querryString = `
      DELETE FROM campaigns
      WHERE campaigns.id = $1;
    `;
  return db.query(querryString, [compaignId]);
};
...
const updateTask = function (userObject) {
  const querryString = `
    UPDATE tasks
...
const createCampaignMessage = function (userObject) {
  let querryString = `
